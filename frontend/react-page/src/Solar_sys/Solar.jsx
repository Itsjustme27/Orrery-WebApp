  ...
  useEffect(() => {
    ...
    function createOrbit(radius) {
      const curve = new THREE.EllipseCurve(
        0,
        0, // Center of the ellipse (sun at (0, 0, 0))
        radius,
        radius, // X radius, Y radius (circular orbit)
        0,
    ...
    function createAsteroid(size, position, color) {
      const geo = new THREE.SphereGeometry(size, 8, 8);
      const mat = new THREE.MeshBasicMaterial({ color: color });
      const mesh = new THREE.Mesh(geo, mat);

      // Position the asteroid within the belt
    ...
    function createAsteroidBelt(numAsteroids) {
      const asteroids = [];
      const beltRadius = 50; // Set your desired belt radius

      for (let i = 0; i < numAsteroids; i++) {
        const angle = Math.random() * Math.PI * 2; // Random angle
        const distance = beltRadius + Math.random() * 10; // Random distance within a small range

        const position = {
          x: Math.cos(angle) * distance,
          y: Math.sin(angle) * distance,
          z: Math.sin(angle) * distance / 180
        };
    ...
    // Call this function to create the asteroid belt
    createAsteroidBelt(100); // Adjust the number of asteroids as needed
    ...
    return () => {
      // Clean up scene and renderer when component unmounts
